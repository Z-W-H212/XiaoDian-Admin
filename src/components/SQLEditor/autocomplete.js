...
export default class Snippets {
  constructor (
    ...
    onInputTableColumn,
    onInputTableAila,
    dbSchema = [{ dbName: '', tables: [{ tblName: '', tableColumns: [] }] }],
  ) {
    this.SORT_TEXT = {
      Database: '0',
      Table: '1',
...
### 注释: ${field.commentName ? field.commentName : ''}
  ...
  getTableNameAndTableAlia (sqlText) {
    ...
    const strList = [
      ...regTableAliaFromList.map(item => item.replace(/(^|(\s+))from\s+/ig, '').replace(/\s+(where|left|right|full|join|inner|union)((\s+.*?$)|$)/ig, '')
        .replace(/\s+as\s+/gi, ' ')
        .trim()),
      ...regTableAliaJoinList.map(item => item.replace(/(^|(\s+))join\s+/ig, '').replace(/\s+on((\s+.*?$)|$)/, '')
        .replace(/\s+as\s+/gi, ' ')
        .trim()),
    ]
    const tableList = []
