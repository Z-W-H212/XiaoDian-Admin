...
import { Space, Input, Badge, message, Popconfirm } from 'antd'
import ProTable, { ActionType } from '@ant-design/pro-table'
import AsyncCheckbox from '@/components/async-checkbox'
import { SearchOutlined, ArrowLeftOutlined } from '@ant-design/icons'
import {
  addFolderRoleAuth,
  cancelFolderRoleAuth,
  addFileRoleAuth,
  cancelFileRoleAuth,
  getRowRule,
  postRowRule,
...
export function UserDataset (props: Props) {
  const { roleId } = props
              ...
              <AsyncCheckbox
                ...
                onRequest={async (value) => {
                  ...
                  if (row.type === 'node') {
                    ...
                    if (value) {
                      await addFolderRoleAuth(params)
                    } else {
                      await cancelFolderRoleAuth(params)
                    }
        ...
        modalVisible?.modal === 'ROW_MODAL' && (
          <RowPermissionSelect
            ...
            onRequest={async (params) => {
              return {
                data: await getRowRule({ roleId, schemaTableId: params.fileId }),
              }
            }}
...

      {
        modalVisible?.modal === 'COL_MODAL' && (
          <ColPermissionSelect
            ...
            params={modalVisible?.params}
            onRequest={async (params) => {
              const result = await getColRule({ roleId, schemaTableId: params.fileId })
              return {
                data: result.map(e => ({
                  id: e.columnId,
                  alias: e.columnAlias,
